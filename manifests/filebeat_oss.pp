...
#   Optional template source path. If set then the template is not downloaded from the mirror!
# @param module_repo
#   Wazuh module repo URL
# @param module_source
#   Optional source path to module in puppet files... instead of offline download.
class wazuh::filebeat_oss (
  ...
  Optional[String] $module_source = undef,
) {
  package { 'filebeat':
    ensure => $version,
  }
  ...
  # Dopin to rewrite the user filebeat runs with... in case we do not run filebeat as root
  $service_overrides  = {
    'Service' => {
  ...
  systemd::dropin_file { 'puppet-filebeat.conf':
